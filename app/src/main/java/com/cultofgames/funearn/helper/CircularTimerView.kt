...
class CircularTimerView : View {
    ...
    private var circularTextSize = 18f
    ...
    private fun initPaints(context: Context, attrs: AttributeSet?) {
        ...
        progressBarPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            strokeWidth = strokeWidthDimension * resources.displayMetrics.density
            strokeCap = if (roundedCorners) {
                Paint.Cap.ROUND
            } else {
                Paint.Cap.BUTT
            }
            val pc = String.format("#%06X", 0xFFFFFF and progressColor)
            color = Color.parseColor(pc)
        }
        progressBarBackgroundPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            color = Color.parseColor(bc)
        }
        backgroundPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.FILL
            color = backgroundColor
            color = Color.parseColor(String.format("#%06X", 0xFFFFFF and backgroundColor))
        }
        ta.recycle()
        textPaint = TextPaint().apply {
            color = progressTextColor
            color = Color.parseColor(String.format("#%06X", 0xFFFFFF and progressTextColor))
            textSize = circularTextSize
            isAntiAlias = true
        }
    }
    ...
    private fun setCountDownTimer() {
        countDownTimer?.cancel()
        countDownTimer = object : CountDownTimer(remainingTime, timeInterval) {
            override fun onTick(l: Long) {
                remainingTime = l
                val percentTimeCompleted = (maxTime - l) / maxTime.toDouble()
                drawUpto = (maxValue * percentTimeCompleted).toFloat()
