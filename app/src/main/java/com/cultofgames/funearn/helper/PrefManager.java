...
import static com.cultofgames.funearn.Just_base.addPoint;
...
import static com.cultofgames.funearn.helper.Constatnt.GET_USER;
import static com.cultofgames.funearn.helper.Constatnt.ID;
import static com.cultofgames.funearn.helper.Constatnt.IS_APPS_ENABLE;
...
import com.cultofgames.funearn.OnScratchComplete;
import com.cultofgames.funearn.R;
import com.cultofgames.funearn.WelcomeActivity;
import com.cultofgames.funearn.csm.ActivitySplash;
import com.cultofgames.funearn.csm.MainActivity;
import com.cultofgames.funearn.csm.topsheet.Coins_Dialog;
...
public class PrefManager {
                ...
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            ContextExtensionKt.showLongToast(t.getContext(), e.getMessage());
                        }
                    }
                },
    ...
    public static void algoratham(Context context, JSONObject data) {
        try {
            ...
            if (data.has("is_apps_enabled")) {
                setString(context, IS_APPS_ENABLE, data.getString("is_apps_enabled"));
            }
        ...
        if (AppController.getInstance().isConnected((AppCompatActivity) context) && !(AppController.getInstance().getId().equals("0"))) {
            JsonRequest jsonReq = new JsonRequest(Request.Method.POST, Base_Url, null,
                ...
                public void onErrorResponse(VolleyError error) {
                    ...
                    context.startActivity(i);
                    ((Activity) context).finish();
                    ContextExtensionKt.showLongToast(context, error.getMessage());
                }
            }) {
                        ...
                        try {
                            if (response.getString("error").equalsIgnoreCase("1")) {
                                if (response.getString("vpn").equals("1")) {
                                    if (vpn()) {
                                        ContextExtensionKt.showShortToast(context, "Please disconnect the vpn and reopen the app!");
                                        ((Activity) context).finish();
                                    } else {
        ...
        JsonRequest stringRequest = new JsonRequest(Request.Method.POST, Base_Url, null,
                new Response.Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            dialogg.dismiss();
                            ContextExtensionKt.showShortToast(contextt, response.getString("message"));
                        } catch (Exception e) {
                            dialogg.dismiss();
                        }
                    }
                ...
                new Response.ErrorListener() {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        ContextExtensionKt.showShortToast(contextt, error.getMessage());
                    }
                }) {
            @Override
            protected Map<String, String> getParams() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            if (response.getString("error").equalsIgnoreCase("false")) {
                                ...
                                ContextExtensionKt.showShortToast(context, "You've already claim your daily bonus!");
                            }
                        } catch (Exception e) {
